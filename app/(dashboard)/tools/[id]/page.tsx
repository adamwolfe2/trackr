...
import { ToolDetailTabsLoader as ToolDetailTabs } from "@/components/tools/tool-detail-tabs-loader";
...
export default async function ToolDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const clerkUsers = clerkUserIds.length > 0
        ? await (await clerkClient()).users.getUserList({ userId: clerkUserIds, limit: 100 }).catch(() => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Failed state banner */}
            {tool.status === "failed" && !report && (
                <div className="border border-neutral-300 bg-neutral-50 p-4 flex items-start gap-3">
            ...
            {/* Fallback report banner */}
            {tool.status === "active" && report?.summary?.includes("Re-run research") && (
                <div className="border border-neutral-300 bg-neutral-50 p-4 flex items-start gap-3">
