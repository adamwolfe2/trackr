...
// No KV_REST_API_URL/TOKEN in test env → falls back to in-memory automatically
...
describe("rateLimit", () => {
  ...
  it("uses separate counters for different identifiers", async () => {
    ...
    await rateLimit(id1, opts);
    const r = await rateLimit(id2, opts);
...
describe("getRateLimitHeaders", () => {
  it("returns correct header format", async () => {
    const result = await rateLimit(uniqueId(), { limit: 10, windowSeconds: 60 });
